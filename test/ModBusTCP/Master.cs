...
namespace ModbusTCP
{
    ...
    public class Master
    {
        ...
        {
            WriteAsyncDataAsync(CreateReadHeader(id, unit, startAddress, numInputs, fctReadCoil), id);
        }
        ...
        {
            WriteAsyncDataAsync(CreateReadHeader(id, unit, startAddress, numInputs, fctReadDiscreteInputs), id);
        }
        ...
        {
            WriteAsyncDataAsync(CreateReadHeader(id, unit, startAddress, numInputs, fctReadHoldingRegister), id);
        }
        ...
        {
            WriteAsyncDataAsync(CreateReadHeader(id, unit, startAddress, numInputs, fctReadInputRegister), id);
        }
        ...
        private async void WriteAsyncDataAsync(byte[] write_data, ushort id)
        {
            if ((tcpAsyCl != null) && (tcpAsyCl.Connected))
            {
                try
                {
                    //System.Net.Sockets.SocketAsyncEventArgs tmp = new SocketAsyncEventArgs();
                    //tcpAsyCl.BeginSend(write_data, 0, write_data.Length, SocketFlags.None, new AsyncCallback(OnSend), null);
                    //var tmp = await tcpAsyCl.(write_data, SocketFlags.None);
        ...
        private void OnReceive(System.IAsyncResult result)
        {
            ...
            else
            {
                ...
                if (data.Length == 1)
                {
                    proxsensor = Convert.ToInt32(data[0].ToString());
                }
            }
            // ------------------------------------------------------------
